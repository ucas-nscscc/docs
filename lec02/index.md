...
* 思考 2：工具之间可以通过_管道_连接，将一个工具的输出作为另一个工具的输入，组合起来完成更复杂的任务。
...
1. 提取提交记录中的 commit SHA 这个任务乍一想不太号实现，观察 `git log` 的输出
...
接下来，我们希望能够像操作数组那样操作 commit SHA 的列表，由于 CMT\_SIGN 中每一个 commit SHA 是用空格隔开的，因此可以利用 shell 中定义数组的语法来将 `CMT_SIGN` 转化为一个数组：
